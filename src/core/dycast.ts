...
export interface CastUser {
  ...
  isFan?: boolean;
  fanBadgeName?: string;
  fanBadgeLevel?: number;
  fanBadgeTitle?: string;
}
...
export class DyCast {
   ...
   */
    ...
    // ======== 从底层 User 中解析粉丝灯牌数据 ========
    const fansClub = (data as any).fansClub?.data as any;

    let isFan = false;
    let fanBadgeName: string | undefined;
    let fanBadgeLevel: number | undefined;
    let fanBadgeTitle: string | undefined;

    if (fansClub) {
      // userFansClubStatus：1 通常表示已经加入粉丝团
      const status = fansClub.userFansClubStatus;
      if (status === 1) {
        isFan = true;
      }

      // clubName：粉丝团名字（例如：童话的粉丝团）
      fanBadgeName = fansClub.clubName;

      // level：粉丝团等级
      if (typeof fansClub.level === 'number') {
        fanBadgeLevel = fansClub.level;
      }

      // badge.title：粉丝灯牌显示称号，如“童话守护 5 级”
      if (fansClub.badge) {
        fanBadgeTitle = fansClub.badge.title;
      }
    }

    // ============ 构造 CastUser ============
    return {
      id: (data as any).secUid || (data as any).idStr,
      name: data.nickname,
      gender: data.gender,
